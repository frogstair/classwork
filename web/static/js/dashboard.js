...
function loadWorkspace() {
    ...
    if(dashboard.headmaster) {
        var template = `<li class="nav-item">
            <p class="nav-link active">Headmaster</p>
        </li>`
        $("#navbar").prepend($(template))
    }
    if(dashboard.teacher) {
        var template = `<li class="nav-item">
            <p class="nav-link active">Teacher</p>
        </li>`
        $("#navbar").prepend($(template))
    }
    if(dashboard.student) {
        var template = `<li class="nav-item">
            <p class="nav-link active">Student</p>
        </li>`
        $("#navbar").prepend($(template))
    }
...
function updateSelection() {
    if(!selected || selected == 0) {
        ...
        Cookies.set("_lsel", selected)
    }

    if(selected == PERMS.Headmaster) {
        dashboard.headmaster.schools.forEach((school) => {
            template = `<div id="${school.id}" class="card mb-3">
            <div class="row g-0">
              <div class="col-sm-8">
                <div class="card-body">
                  <h5 class="card-title mt-4">${school.name}</h5>
                </div>
              </div>
              <div class="col-sm-4 mt-4 mb-4">
                  ...
                  <div class="row">
                    <div class="col">
                      <button onclick="deleteSchool('${school.id}')" class="btn btn-primary">Delete</button>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div class="col-2 d-grid">
          <button id="schooladder" onclick="addSchool()" class="btn btn btn-primary">
...
function addSchool() {
    ...
    $("#schooladder").prepend($("<span id=\"waiter\" class=\"spinner-grow spinner-grow-sm\" role=\"status\" aria-hidden=\"true\"></span>"))
    $("#plus").remove()
